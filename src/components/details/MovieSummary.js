...
const MovieSummary = ({ movie }) => {
  return (
    <div className="flex md:flex-row flex-col items-start my-10 w-full h-full">
      <video className="md:w-1/2" poster={movie.poster} preload="none" controls>
        <source src={exampleVideo} />
      </video>
      <div className="flex flex-col h-full md:w-1/2">
        <div className="md:my-4 my-4 md:mr-4">
        ...
        <div className="md:my-0 my-4 md:mr-4">
          <div className="flex items-center mb-2">
            <FilmIcon className="w-6 ml-2" />
            <h2 className="text-xl font-yekan-bold">درباره فیلم</h2>
          </div>
          <p className="text-sm">{movie.about}</p>
        </div>
      </div>
    </div>
  );
